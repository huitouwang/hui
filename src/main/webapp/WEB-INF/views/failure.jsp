...
<body>
    ...
    <div id="wrapper">
        <div class="chengeOK"><span><img src="resources/images/failure.jpg" width="63"  height="55"/></span>
    <label>操作失败！${error}
    	<c:if test="${!empty identy}">
    		<c:choose>
    			<c:when test="${identy eq 1}">
    				该邮箱已激活！
    			</c:when>
    			<c:when test="${identy eq 2}">
    				该邮箱激活时效已过，你可以登录p2p网选择重新发送激活链接！
    			</c:when>
    		</c:choose>
    	</c:if>
    </label></div>
        <div>
        	<p>如果无法跳转请点击这里：<a style="color:#ff6600;" href="<%=basePath%>">返回首页</a></p>
            <a class="link" href="/" onclick="history.go(-1)"><span id="sec">5</span> 秒后将关闭此页面</a>
			<br /><br /><br />
        </div>
    </div>
