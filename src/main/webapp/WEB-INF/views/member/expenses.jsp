...
<h1 class="vipContTitle"><span><img src="resources/images/vipseversicon.png" /></span>尊敬的p2p网用户，以下是您在p2p网的收支记录，敬请仔细审阅：</h1>
<p>收支单记录了您各种交易产生的支出和收入的明细，请选择事件类型和时间，然后点击“查询”按钮浏览。</p>
</div>
...
<table cellpadding="0" cellspacing="0" border="0" class="vipSeversTable">
 <thead>
  <tr><th>日期</th><th>类型</th><th>支出</th><th>收入</th><th>说明</th></tr>
 </thead>
 <tbody>
 <c:forEach items="${expenses}" var="expense">
  ...
  <td>${expense[1]}</td>
  <td>${expense[2]}</td>
  <td><fmt:formatNumber value="${expense[3]}" type="currency" /></td>
  <td><fmt:formatNumber value="${expense[4]}" type="currency" /></td>
  <td>${expense[5]}</td>
  </tr>
 </c:forEach>
