...
<!-- 							<li><a href="loanManage/hasTheRepaymentLoan.htm">已还款借款</a></li> -->
					...
					<div class="vipContBox">
						<div class="vipContTitleBox">
							...
							<p>
								您目前审核中的借款总额是：<font>￥${money}</font>，共 <font>${page.totalCount}</font>
								笔审核中借款。
							</p>
						</div>
							...
							<table cellpadding="0" cellspacing="0" border="0"
								...
								<c:forEach items="${page.list}" var="pursue">
									<tbody>
										<tr>
<%-- 											<td>${pursue[0]}</td> --%>
											<td><a href="loaninfo/getLoanInfo.htm?id=${pursue[9]}">${pursue[0]}</a></td>
											<td>${pursue[2]}</td>
											...
											<c:if test="${pursue[6]>0}">
												<td>${pursue[6]}天</td>
											</c:if>
											<c:if test="${empty pursue[5] && empty pursue[6] }">
												<td>满标回款</td>
											</c:if>
											<td>${pursue[7]}</td>
											<td><fmt:formatNumber value="${pursue[8]*100}"
													pattern="##.##" minFractionDigits="2" />%</td>
										</tr>
									</tbody>
								</c:forEach>
