					...
					<div class="vipContBox">
						<div class="vipContTitleBox">
							<h1 class="vipContTitle">
								<span><img src="resources/images/vipseversicon.png" /></span>尊敬的p2p网用户，下列收藏列表能为您快捷服务。
							</h1>
						</div>
						<form>
							<table cellpadding="0" cellspacing="0" border="0"
								...
								<tbody>
									<c:forEach items="${collects}" var="collect" varStatus="no">
										<tr>
											<td><a href="loaninfo/getLoanInfo.htm?id=${collect[10]}">${collect[1]}</a></td>
											<td>${collect[2]}</td>
											<td>${collect[3]}</td>
											<td>
											<fmt:formatNumber value="${collect[4]}" type="currency" />
											</td>
											<td><fmt:formatNumber value="${collect[5]}" pattern="##.##" />%</td>
											<td>
											<c:if test="${collect[12] eq 1}">
												${collect[6]}个月
											</c:if>
											<c:if test="${collect[12] eq 2}">
												${collect[11]}天
											</c:if>
											<c:if test="${collect[12] eq 3}">
												秒标
											</c:if>
											<c:if test="${collect[12] eq 4}">
												流转标
											</c:if>
											</td>
											<td>
