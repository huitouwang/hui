...
<style type="text/css">
	...
	.pag_s_1{ background:url(/resources/images/page_r.png) repeat-x;background-color:#F98100;  padding:0px 7px; border-bottom:1px solid #BE2525; border-left:1px solid #D3514F; border-right:1px solid #BE2525; border-top:1px solid #D3514F; color:#fff; font-weight:bold; }
</style>
<table cellpadding="0" cellspacing="0" border="0" class="vipSeversTable">
              <tr>
                <th class="tab_detail">编号</th>
                <th class="tab_detail">标题</th>
                <th class="tab_detail">类型</th>
                <th class="tab_detail">年化利率</th>
                <th class="tab_detail">还款方式</th>
                <th class="tab_detail">借款期限</th>
                <th class="tab_detail">下期还款时间</th>
                <th class="tab_detail">下期还款金额</th>
              </tr>
              <c:forEach var="item" items="${planDetail}" varStatus="s">
	              <tr>
	                <td>${item.loanNumber }</td>
	                <td>${item.loanTitle }</td>
	                <td>${enum_loansign_type[item.loanType]}</td>
	                <td>${item.rate }</td>
	                <td>${enum_loansign_refundway[item.refundWay]}</td>
	                <td>${item.month}${item.loanType == 2 || item.loanType == 3 ? '(天)' : '(月)' }</td>
	                <td>${item.preRepayDate }</td>
	                <td ><fmt:formatNumber value="${item.bjAndLx.total}" type="currency" /></td>
	              </tr>
              </c:forEach>
              <tr>
	           	<td colspan="8" >
