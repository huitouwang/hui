			...
			<div class="vipContentBox">
				...
				<div class="vipRightBox">
					<div class="vipHeadMenuBox">
						<ul>
							<li><a href="<%=basePath%>depositshistory/init_two"
								class="vipHeadLink">投标中项目</a></li>
					...
					<div class="vipContBox">
						<div class="vipContTitleBox">
							<h1 class="vipContTitle">
								<span><img src="../resources/images/vipseversicon.png" /></span>尊敬的p2p网用户，以下是您在p2p网的竞标中投资。
							</h1>
							<p>
								您目前竞标中的投资总额是：<font><fmt:formatNumber value="${count_money}" type="currency"/></font>，共 <font>${count}</font>
								笔投标。
							</p>
						</div>
						<form>
							<table cellpadding="0" cellspacing="0" border="0"
								...
								<c:forEach items="${loanrecordlist}" var="loanrecord">
									<tr>
										<td><a href="loaninfo/getLoanInfo.htm?id=${loanrecord.loanId}">${loanrecord.loanNumber}</a></td>
										<td>${loanrecord.loanTitle}</td>
										<td>${loanrecord.borrowername}</td>
										<td><fmt:formatNumber value="${loanrecord.issueLoan}" type="currency"/></td>
										<td><fmt:formatNumber value="${loanrecord.tenderMoney}" type="currency"/></td>
										<td>${loanrecord.lastDays }</td>
										<td><fmt:formatNumber value="${loanrecord.schedule}"
												pattern="0" />%</td>
									</tr>
									</c:forEach>
								</tbody>
