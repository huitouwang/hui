...
if(idx==3){
$('.loanTitle').text(objarr[idx]['loanTitle']);
$('.refundWay').text(objarr[idx]['refundWay']);
$('.rate').text(objarr[idx]['rate']);
$('.issueLoan').text(parseFloat(objarr[idx]['issueLoan']));
$('.month').text(parseFloat(objarr[idx]['month']));
$('.reward').text(parseFloat(objarr[idx]['reward']));
$('.loading.one span').attr('data-value',(parseFloat(objarr[idx]['successfulLending'])/(parseFloat(objarr[idx]['successfulLending'])+parseFloat(objarr[idx]['remainingCopies'])))*100);
$('.remainingCopies').text((parseFloat(objarr[idx]['successfulLending'])/(parseFloat(objarr[idx]['successfulLending'])+parseFloat(objarr[idx]['remainingCopies'])))*100);
}
}); 
}); 
...
<c:if test="${a.deputysection.id  == 17}">
    ...
    <ul><li class="on"><h1>借款列表</h1></li></ul>
					<!--a href="loaninfo/openLoan.htm">更多借款项目 &gt;&gt;</a-->
				</div>
				 <!--借款信息-->
			<div class="contwrapbox">
	        <dl>
	 	    <dt><span class="loanTitle"></span><span class="refundWay"></span></dt>
	 	    <dd>
	        ...
	        <tr>
	        <td><b class="h rate"></b>%</td>
	        <td>
            <b class="h issueLoan"></b>元	        
            </td>
	        <td>
			<b class="month"></b>月
	        </td>
	        <td><b class="reward"></b>%</td>
	        </tr>
	 	    </table>
	 	    <div class="dadaloadbox">
			投资进度：<div class="loading one"><span data-value=""></span>
		    </div>
		    <b class="remainingCopies"></b>%
		    </div>
		<div class="textbox">
		<table width="100%">
		<tr>
		<td>募集结束：<span class=""></span></td>
		<td>还款方式：<span class=""></span></td>
		<tr>
		<td>还款日期：<span class=""></span></td>
		<td>第三方担保：<span class=""></span></td>
		</tr>
		</table>
		<div class="clear"></div>
		</div>
...
$('.right-button i').click(function(){
	...
	$(function(){
		...
		$(function(){
		$(".dadaloadbox .loading.one span").load("javascript:void(0)",function(){
			var $data_value=$(this).attr("data-value");					   
				$(this).animate({
				width:$data_value+"%"	
						});	
