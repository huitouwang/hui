$(document).ready(function(){
	...
	$("#editLoansign").click(function(){
		...
		if(selections==null || selections=="" || selections==undefined || selections.length<1){
			...
			navTab.openTab("updateloanSign", "/loanSign/seeDetails?operNumber=2&id="+selections[0].id, { title:"编辑", fresh:true});
		}
	});
	...
	$("#queryLoansign").click(function(){
		...
		if(selections==null || selections=="" || selections==undefined || selections.length<1){
			...
			//打开编辑页面属性依次为：窗体id，路径、标题，重复打开是否刷新
			navTab.openTab("queryloanSign", "/loanSign/seeDetails?operNumber=3&id="+selections[0].id, { title:"查看详情", fresh:true});
		}
	});
	...
	/***************按钮及点击事件************/
		...
		$("#EXPloansign").click(function(){
	        window.location.href='/loanSign/outPutExcel';
		});
     ...
     //将选择的记录的id传递到后台去并执行delete操作
       ...
       $.ajax({
		...
		$("#EXPLoanRecord").click(function(){
            ...
            var toDeleteRecordID=selections[0].id;
            window.location.href='/baseLoanSign/outPutLoanrecordExcel?id='+toDeleteRecordID;
		});
		...
		//流标
		$("#LBloansign").click(function(){
			var selections=$('#mygrid1').omGrid('getSelections',true);
            if (selections.length == 0) {
            	alertMsg.info("请选择要进行流标的记录！");
                return false;
            }
            alertMsg.confirm('是否对选中的标进行流标操作?', {
                okCall: function(){
                	var toDeleteRecordID=selections[0].id;
                	$.ajax({
                		data:'id='+toDeleteRecordID,
                		url:'/baseLoanSign/flow.htm',
                		type:'post',
                		success:function(msg){
                			if(msg==1){
                				$('#mygrid1').omGrid('reload');//刷新当前页数据
                				...
                				alertMsg.info("流标失败,只有进行中标可以流标,请尝试刷新页面！");
                			}else if(msg==3){
                				alertMsg.info("流标失败,该标未满标,请尝试刷新页面！");
                			}else{
                				alertMsg.info("流标失败,请尝试刷新页面！");
                			}
                		}
                	});
                }
	...
	showmygrid5(0);
	 ...
	 $("#loanCategory").omCombo({
      ...
      $('span#button-search').click(function(){
    	  $('#mygrid1').omGrid('setData', '/loanSign/loanSignList?'+$("#searchfrom").serialize());
	...
	$('#mygrid1').omGrid({
        ...
        colModel : [ 
                     ...
                     {header : '流标时间', name : 'flotTime', align : 'center', width : 110},
