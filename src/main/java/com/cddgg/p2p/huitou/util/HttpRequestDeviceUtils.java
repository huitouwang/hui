 ...
 * @version:    1.0   
 * @since:  JDK 1.7.0_25  
 * Create at:   2014年2月7日 下午3:56:39   
 * Description:  测试用来判断用户是通过手机访问网站还是电脑
 *   
 */
    ...
    public static boolean isMobileDevice(HttpServletRequest request){
        ...
        for (int i = 0; via!=null && !via.trim().equals("") && i < mobileGateWayHeaders.length; i++) {
            if(via.contains(mobileGateWayHeaders[i])){
                mobileFlag = true;
                for (int j = 0;!mobileFlag && userAgent!=null && !userAgent.trim().equals("") && j < mobileUserAgents.length; j++) {
                    if(userAgent.contains(mobileUserAgents[j])){
                        mobileFlag = true;
                        break;
                    }
                }
