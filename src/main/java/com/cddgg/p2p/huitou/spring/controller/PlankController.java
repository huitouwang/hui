...
public class PlankController {
	 ...
	 * @param loanUnit
	 *            单份金额
	 * @return <0>用户未登录 <1>用户输入的不是数字 <2>用户输入的数字不正确 <3> 用户输入的金额大于了当前用户的金额 <8>
	 *         用户输入金额大于了该标的金额 <8> 所有条件正确
	 */
	@ResponseBody
	@RequestMapping("ajaxLoanInfo.htm")
	public String getAjaxLoanInfo(Long id, String money, Double loanUnit,
	...
	public synchronized String getLoanInfo(Long id, Double money,
		...
		try {
			BidInfo bid = new BidInfo(userinfo, DateUtils.format("yyyy-MM-dd"),
					money, loan);
			bid.setpTTrdFee(df.format(borrowerFundService.getManagement(money,
					loan)));
			...
			record.setEndTime(DateUtils.add("yyyy-MM-dd HH:mm:ss",
					DateUtils.format("yyyy-MM-dd HH:mm:ss"), Calendar.MINUTE,
					Constant.time));
			...
			loans.setMgtMoney(Double.parseDouble(bid.getpTTrdFee()));
			
			plankService.update(loanrecord, account, money, loans,
					bid.getpMerBillNo());
			/*
			 * String xml = ParseXML.bidXml(bid); Map<String,String> map =
			 * RegisterService.registerCall(xml);
			 * 
		...
		request.getSession().setAttribute(
				Constant.SESSION_USER, userinfo);
		request.setAttribute("url",
				"/depositshistory/init_two");
