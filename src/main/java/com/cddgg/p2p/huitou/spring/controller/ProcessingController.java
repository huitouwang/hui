...
public class ProcessingController {
			...
			if (ParameterIps.pianText(returnInfo)) {
				try {
					...
					if (bool) {
						...
						request.getSession()
								.setAttribute(
										com.cddgg.p2p.huitou.constant.Constant.SESSION_USER,
										userInfoServices
												.queryBasicsInfoById(registerInfo
														.getpMemo1()));
				...
				LOG.error("环讯注册失败->失败原因:该处理结果不是由环迅返回"
						+ returnInfo.getP3DesXmlPara());
			...
			if (ParameterIps.pianText(returnInfo)) {
				try {
					...
					if (null == userbasics.getUserfundinfo().getpIdentNo()) {
						boolean bool = processingService.registration(
								registerInfo, userbasics);
						if (!bool) {
							LOG.error("环讯注册成功->平台数据处理失败->用户开户流水号:"
									+ registerInfo.getpMerBillNo() + " 开户环讯账号:"
									+ registerInfo.getpIpsAcctNo() + " 开户时间:"
	...
	public synchronized String asynchronismRecharge(HttpServletRequest request) {
		...
		LOG.info("支付充值异步返回处理开始！");

		String queryString = request.getQueryString();

		String url = request.getRequestURL().toString();

		LOG.info("queryString:" + queryString + ",url:" + url);
		...
		if (succ.equals(Constant.SUCCESS)) {
			...
			LOG.error("环迅充值失败--》失败原因:" + request.getParameter("msg"));
			request.setAttribute("error",
					"环迅充值失败--》失败原因:" + request.getParameter("msg"));
			return "WEB-INF/views/failure";
		}
	}
	...
	public synchronized String repayment(ReturnInfo returnInfo,
		...
		if ((Constant.SUCCESS).equals(returnInfo.getpErrCode())) {
			if (ParameterIps.pianText(returnInfo)) {
				...
				try {
					info = (RepaymentReturnInfo) XmlParsingBean
							.simplexml1Object(returnInfo.getP3DesXmlPara(),
									new RepaymentReturnInfo());
				...
				Repaymentrecord repaymentrecord = baseLoansignService
						.getRepaymentId(Long.parseLong(info.getpMemo1()));
				// 获取借款人还款资金信息=====还需要得到该用户是否为特权用户
				ExpensesInfo expensesInfo = new ExpensesInfo();
				if (Integer.parseInt(info.getpMemo2()) == 1) {
					expensesInfo = borrowerFundService.advanceRepayment(
							repaymentrecord.getLoansign(), null);
				} else {
					expensesInfo = borrowerFundService
							.getBorrowerFund(repaymentrecord);
				}
				// ---------------------修改当前期的还款情况
				processingService.setRepayment(repaymentrecord,
						Integer.parseInt(info.getpMemo2()), info, expensesInfo);
				// ----------------------
			...
			if (ParameterIps.pianText(returnInfo)) {
				...
				try {
					...
					Userbasicsinfo userbasicsinfo = userInfoServices
							.queryBasicsInfoById(withInfo.getpMemo1());
					...
					if (num <= 0) {
						...
						boolean bool = processingService.withdrlRecord(
								withInfo, userbasicsinfo);
						if (!bool) {
							try {
								financialExceptionNotes
										.note(ENUM_FINANCIAL_EXCEPTION.RECHARGE,
	...
	public synchronized void repaymentAsynchronous(ReturnInfo returnInfo) {
		...
		if (Constant.SUCCESS.equals(returnInfo.getpErrCode())) {
			...
			if (ParameterIps.pianText(returnInfo)) {
				// 将还款信息解析成对象
				RepaymentInfo info = RegisterService.repaymentXml(returnInfo
						.getP3DesXmlPara());
				// 判断数据是否已处理
				int num = processingService
						.accountInfoNum(info.getpIpsBillNo());
				if (num <= 0) {
					// 得到还款信息
					Repaymentrecord repaymentrecord = baseLoansignService
							.getRepaymentId(Long.parseLong(info.getpMemo1()));
					...
					// ----------------------
					try {
						processingService.getrepayment(info, repaymentrecord,
								expensesInfo);
					} catch (Exception e) {
						e.printStackTrace();
						try {
							...
							LOG.error("环讯还款成功->平台数据处理失败->借款人编号:"
									...
									+ "还款流水号:" + info.getpMerBillNo()
									+ "环讯还款流水号:" + info.getpIpsBillNo());
	...
	public synchronized void asynchronismLoans(ReturnInfo returnInfo,
		...
		if (Constant.SUCCESS.equals(returnInfo.getpErrCode())) {
			if (ParameterIps.pianText(returnInfo)) {
				...
				// 获取标的情况
				Loansign loan = loanSignQuery.getLoansignById(auditInfo
						.getpMemo1());
				// 判断数据是否已处理
				int num = processingService.accountInfoNum(auditInfo
						.getpIpsBillNo());
				if (num <= 0) {
					// 对借款人账户的处理
					boolean bool = processingService.tenderAudit(loan
							.getUserbasicsinfo(), auditInfo, request, loan
							.getLoansignbasics().getMgtMoney());
					if (!bool) {
						Log.error("环迅放款成功-->我方放款失败-->数据处理失败-->还款ips编号:"
								+ auditInfo.getpIpsBillNo() + "还款标号:"
								+ loan.getId());
					}
				}
				// 当标的类型不为流转标的时候生成还款计划
		...
		if (info.getpErrCode().equals(com.cddgg.base.constant.Constant.SUCCESS)) {
			if (ParameterIps.pianText(info)) {
				...
				try {
					...
					// 判断数据是否已处理
					int num = processingService.accountInfoNum(bid
							.getpIpsBillNo());
					if (num > 0) {
						// 获取当前用户的最新信息
						...
						BalanceInquiryInfo money = RegisterService
								.accountBalance(userinfo.getUserfundinfo()
										.getpIdentNo());
						// 获取标的详细信息
						Loansign loan = loanSignQuery.getLoansignById(bid
								.getpMemo1());
						...
						loanrecord.setTenderMoney(Double.parseDouble(bid
								.getpTrdAmt()));
						...
						account.setExpenditure(Double.parseDouble(bid
								.getpTrdAmt()));
	...
	// StringBuffer("-----BEGIN PUBLIC KEY-----").append(crlf);
