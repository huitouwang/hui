...
public class LoanInfoController {
	...
	public String getLoanInfo(String id, HttpServletRequest request) {
		...
		if (StringUtil.isNotBlank(id) && StringUtil.isNumberString(id)) {
			...
			// 获取标的认购金额
			request.setAttribute("scale",
					loanSignQuery.getLoanrecordMoneySum(idNum));
	...
	public String getLoanlist(HttpServletRequest request, String money,
			String month, String type, String rank, String loanType, Integer no) {
		PageModel page = new PageModel();
		page.setPageNum(no);
		page = loanManageService.getLoanList(money, month, type, rank,
				loanType, page);
