...
public class WithdrawAdminController {
	...
	public String queryPage(@ModelAttribute("PageModel") PageModel page,
		...
		Integer count = withdrawAdminService.queryCount(beginDate, endDate,
				userName);
		page.setTotalCount(count);
		List list = withdrawAdminService.queryPage(page, beginDate, endDate,
				userName);
	...
	@ResponseBody
	public String checkPass(String ids, HttpServletRequest request) {
		...
		try {
			...
			String amt = str[3].toString(); // 提现金额

			String user_id = str[9].toString(); // 用户id
			
			String state = str[10].toString();
			
			if("1".equals(state)){
				return "已审核通过，不能重复审核";
			}

			Boolean bool = withdrawAdminService.checkPass(ids, amt, user_id);
	...
	public void exportExcel(String ids, HttpServletRequest request,
		...
		String[] headers = new String[] { "姓名", "用户名", "提现金额", "提现费", "订单号",
				"ips订单号", "提现时间", "备注" };
		// 数据源
		List list = withdrawAdminService.queryById(ids);
		...
		// 导出充值记录
		rechargeModel.downloadExcel("充值记录", null, headers, content, response,
				request);
	}
}