...
public class LoanSignFundImpl implements LoanSignFund {
	...
	public BigDecimal managementFee(BigDecimal money, Loansign loan,
		...
		if (loan.getLoanType() == 1) {
			// 判断该标的时间是否大于2个月
			if (loan.getMonth() > 2) {
				System.out.println(loan.getOther() + (loan.getMonth() - 2)*loan.getOthers());
				managementFee = money.multiply(new BigDecimal(loan.getOther() + (loan.getMonth() - 2)*loan.getOthers()));
			} else {
				managementFee = money.multiply(new BigDecimal(loan
						.getOther()));
			}
		} else if(loan.getLoanType() == 2) {
			managementFee = money.multiply(new BigDecimal(loan.getOther()*loan.getUseDay()));
		}
		// 判断该会员是否为特权会员
		if (isPrivilege == 1) {
			if (managementFee.doubleValue() > loan.getHighLines()) {
				managementFee = new BigDecimal(loan.getHighLines());
			}
		}
	...
	public List<CalculateLoan> loanCalculate(BigDecimal issueLoan, double rate,
		...
		for (int i = 1; i <= periods; i++) {
			...
			// 将本期未还本金，作为下一期的本金
			BigDecimal issueloan = calculoan.getNotReturn();
