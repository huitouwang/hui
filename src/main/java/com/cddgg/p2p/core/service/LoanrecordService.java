...
public class LoanrecordService {
	 ...
	 * @return
	 */
	public List<UserRank> getRank(int flag) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<UserRank> getTotalRank() {

		String sql = "select b.userName,a.money from (select userbasicinfo_id,sum(tenderMoney) as money  from loanrecord where isSucceed = '1'  group by userbasicinfo_id) a,userbasicsinfo b where a.userbasicinfo_id = b.id order by a.money desc limit 0,5";

		List<UserRank> listRecord = (List<UserRank>) dao.findBySql(sql);
	...
	@SuppressWarnings("unchecked")
	public List<UserRank> getMonthRank() {

		String sql = "select b.userName,a.money from (select userbasicinfo_id,sum(tenderMoney) as money  from loanrecord where isSucceed = '1'  and  year(tenderTime)=year(now()) and month(tenderTime)=month(now())  group by userbasicinfo_id) a,userbasicsinfo b where a.userbasicinfo_id = b.id order by a.money desc limit 0,5 ";
	...
	@SuppressWarnings("unchecked")
	public List<UserRank> getWeekRank() {
		String sql = "select b.userName,a.money from (select userbasicinfo_id,sum(tenderMoney) as money  from loanrecord where isSucceed = '1'  group by userbasicinfo_id) a,userbasicsinfo b where a.userbasicinfo_id = b.id order by a.money desc limit 0,5";
